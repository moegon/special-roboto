...
async function ingestMedia(): Promise<void> {
    ...
    async (progress) => {
      ...
      for (const uri of files) {
        ...
        try {
          ...
          vscode.window.showErrorMessage(`Failed to ingest ${uri.fsPath}: ${(error as Error).message}`);
        }
        completed += 1;
  ...
  vscode.window.showInformationMessage(`Ingested ${files.length} file(s) with Atlas.`);
}
...
async function analyseClip(item?: MediaItem | AtlasClip): Promise<void> {
  ...
  try {
    ...
    vscode.window.showErrorMessage(`Model analysis failed: ${(error as Error).message}`);
  }
}
...
async function resolveClip(item?: MediaItem | AtlasClip): Promise<AtlasClip | undefined> {
  ...
  return libraryProvider.pickClip();
}
