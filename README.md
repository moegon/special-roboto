...
└── media/atlas-icon.svg     # Activity bar icon
```
...
3. **Launch in VS Code**
   ...
   - The **Atlas** icon will appear in the Activity Bar with a *Media Library* view.
...
The Vite dev server proxies `/api/*` requests to `http://localhost:8080` by default. Adjust this in `vite.config.ts` or via the in-app settings panel to match your deployment.
...
Compiled assets land in `admin-console/dist/` and can be served via any static host or folded into a container image alongside your Atlas services.
